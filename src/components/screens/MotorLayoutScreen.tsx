...
export default function MotorLayoutScreen() {
  ...
  const [localLayout, setLocalLayout] = useState({
    ...
    wheelMode: motorLayout.wheelMode,
  });
  ...
  const handleSlotChange = useCallback((wheelIdx: number, slot: number) => {
    setLocalLayout(prev => {
      ...
      return { ...prev, mapMot, wheelMode: prev.wheelMode };
    });
    setSaved(false);
  }, []);
  ...
  const handleInvToggle = useCallback((wheelIdx: number) => {
    setLocalLayout(prev => {
      ...
      return { ...prev, motInv, wheelMode: prev.wheelMode };
    });
    setSaved(false);
  }, []);
  ...
  const handleReset = useCallback(() => {
    const defaults = { mapMot: [0, 1, 2, 3], motInv: [0, 0, 0, 0], wheelMode: localLayout.wheelMode };
    setLocalLayout(defaults);
    setSaved(false);
  }, [localLayout.wheelMode]);
